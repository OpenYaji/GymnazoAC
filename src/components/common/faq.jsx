...
// Sample data for the FAQs (No change to data)
const faqData = [
...
const FAQ = () => {
  ...
  // This split is not used in the final version but kept here for logic history
  const halfLength = Math.ceil(faqData.length / 2);
  const col1 = faqData.slice(0, halfLength);
  const col2 = faqData.slice(halfLength);

  return (
    // Set a light background for light mode, transparent for dark mode (it will inherit from parent)
    <div className='bg-white dark:bg-transparent transition-colors duration-300 p-4 h-full rounded-xl'> 
      <div className='w-full mx-auto'>
        
        {/* Header Section (FAQ title for the column) */}
        <div className='mb-8'>
          {/* FIXED: Added text color for light mode (text-[#5B3E31]) */}
