...
import LogoutConfirmationModal from './logoutConfirmation'; // Adjust path if needed
...
          isActive && item.name !== 'Logout' ? "bg-[#F3D67D] text-black font-bold" : "hover:bg-white/10"
        ...
        <aside className="h-screen bg-[#4D4135] dark:bg-slate-900 text-white flex flex-col w-64 p-4">
            ...
            <nav className="flex-1 flex flex-col justify-between">
              <ul className="space-y-2">
                {menuItems.map((item) => (
                  <NavLink key={item.name} item={item} isActive={location.pathname === item.path} isMobileLink={true} />
                ))}
              </ul>
              <ul className="space-y-2">
            ...
            <div className="flex flex-col h-full items-center p-3 gap-3">
              <div className="flex flex-col items-center space-y-4 bg-[#3C2F2F] dark:bg-slate-800 p-3 rounded-3xl shadow-lg flex-1 w-full">
                ...
                <nav>
                  <ul className="space-y-2">
                    {menuItems.map((item) => (
                      <NavLink key={item.name} item={item} isActive={location.pathname === item.path} isCollapsed={true} />
                    ))}
                  </ul>
                </nav>
              </div>
            ...
            <div className="flex flex-col justify-between h-full p-3">
              <div className="bg-[#3C2F2F] dark:bg-slate-800 rounded-3xl p-4 flex-1 flex flex-col shadow-lg">
                ...
                <nav className="flex-1">
                  <ul className="space-y-2">
                    {menuItems.map((item) => (
                      <NavLink key={item.name} item={item} isActive={location.pathname === item.path} />
                    ))}
                  </ul>
                </nav>
              </div>
